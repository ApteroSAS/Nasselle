...
export type ProvideType = "fake"|"lb4"|"firebase"|'custom-rest-visitors'
const resourceResolver: Record<string,ProvideType> = {
    customers: 'fake',
    users: 'firebase',
    names: 'firebase',
    virtualpanel:'custom-rest-visitors',
};
...
export const multiDataProvider = new Proxy<DataProvider>(defaultDataProvider, {
    get: (target, action) => {
        ...
        return async (resource: string, params: any) => {
            ...
            let provider = null;
            if (resourceResolver[resource] === 'firebase') {
                provider = firebaseDataProvider[action];
            } else {
