...
export const DockerSetup = () => {
  ...
  useEffect(() => {
    ...
    loadProviderDetails();
  }, [userid, dataProvider, notify]);

  const dockerLineConfig = `docker network create nasselle && docker run -d --cap-add NET_ADMIN -e PROVIDER="${provider}" --network nasselle --name mesh-router-nsl.sh nasselle/mesh-router && docker run -d -e DATA_ROOT=/c/DATA -e REF_NET=nasselle -e REF_PORT=80 -e REF_DOMAIN=${refDomain} -v C:\\DATA:/DATA --network nasselle -v /var/run/docker.sock:/var/run/docker.sock --name casaos nasselle/casa-img`;
