...
export function useRefDomain(): {
        ...
        const loadProviderDetails = async () => {
            try {
                ...
                if (domainName && serverDomain) {
                    ...
                    await dataProvider.update(ResourceKey, {id: userid, data: {domainName: initialDomainName, serverDomain: 'nsl.sh'}} as any);
    ...
    return {
        ...
        setDomainName:async (domainName: string) => {
            await dataProvider.update(ResourceKey, {id: userid, data: {domainName}} as any);
            setDomainName(domainName)
        },
        setServerDomain:async (serverDomain: string) => {
            await dataProvider.update(ResourceKey, {id: userid, data: {serverDomain}} as any);
            setServerDomain(serverDomain)
        }
    };
}